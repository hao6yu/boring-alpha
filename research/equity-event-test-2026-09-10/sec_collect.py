...
POLICY_SHA = '3bda0898cd02318535333636b2416deb035d2adc1bf401f67f5c5655daea19a3'
...
class Fetcher:
    ...
    def __init__(self, offline=False):
        ...
        for path in [OLD / 'sec-sources.json', HERE.parent / 'equity-event-repair-2026-09-10/dtss-sources.json']:
            m = json.loads(path.read_text())
            for u, r in m.get('sources', {}).items():
                if r.get('status') == 200:
                    self.cache_sources[u] = {**r, 'cache_manifest': str(path.relative_to(ROOT)), 'cache_manifest_sha256': digest(path)}
    ...
    def get(self, url):
        ...
        with gate:
            ...
            if u.path.lower().endswith('.txt'): raise SourceFailure('TXT_format_disabled_no_request')
...
def selection(fetcher, workers):
    ...
    out.update(selection_status='IN_PROGRESS', cutoff='acceptance_eastern < 2019-09-30T00:00:00-04:00',
               cohort_id='fixed-2019Q3-incumbents-v1', no_future_price_selection=True)
    ...
    with ThreadPoolExecutor(max_workers=workers) as pool:
        ...
        i = start
        while i < len(ranked) and len(out['issuers']) < 100:
            ...
            out['updated_at'] = now()
            out['selection_status'] = 'BLOCKED_EARLIER_RANK' if r['decision'] == 'UNRESOLVED' else 'COMPLETE_100' if len(out['issuers']) == 100 else 'IN_PROGRESS'
            atomic(path, out)
            ...
            if r['decision'] == 'UNRESOLVED' or len(out['issuers']) == 100: break
            nxt = i + workers
...
def collect_issuer(fetcher, issuer):
    ...
    for row in rows:
        if row['form'] == '8-K' and '2.02' in row.get('items', ''):
            try: events[row['accessionNumber']] = event_override(client.event(cik, row))
            except StopCollection: raise
...
def collect_events(fetcher, selected, workers):
    if selected['selection_status'] != 'COMPLETE_100': raise StopCollection('cohort_not_frozen')
    out = {'policy_sha256': POLICY_SHA, 'selection_sha256': digest(HERE / 'sec-selection.json'),
           ...
           'fixed_slots': 1600, 'slots': [], 'status': 'IN_PROGRESS', 'issuers_completed': 0}
    by_cik = {}

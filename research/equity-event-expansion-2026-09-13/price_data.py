...
RAW = ROOT / 'data/snapshots/equity-event-expansion-2026-09-13/prices'
...
def main():
    ...
    assert 0 < len(issuers) <= 200 and len({x['cik'] for x in issuers}) == len(issuers)
    assert all(x.get('membership_final_in_resolved_prefix') is True for x in issuers)
    # A resolved rank prefix may be acquired while later 2019 ranks are still
    # screened. Each included new issuer needs original index/cover ownership.
    owners = json.loads((HERE / 'cohort-prefix-audit.json').read_text())
    assert owners['policy_sha256'] == policy_sha
    assert owners['inherited_100_issuers_exactly_preserved']
    audited = owners['rows']
    assert [x['cik'] for x in issuers[100:100+len(audited)]] == [x['cik'] for x in audited]
    verified_prefix = []
    for row, issuer in zip(audited, issuers[100:]):
        if not row['verified']:
            break
        assert symbol(row['symbol']) == symbol(issuer['historical_symbol'])
        verified_prefix.append(issuer)
    acquisition_issuers = verified_prefix + (issuers[:100] if len(verified_prefix)==100 else [])
    requests = ['SPY'] + [symbol(x['historical_symbol']) for x in acquisition_issuers]
    ...
    manifest['owner_verified_acquisition_prefix'] = [
        {'cik': x['cik'], 'historical_symbol': x['historical_symbol']}
        for x in issuers[:100] + verified_prefix]
    manifest['owner_audit_sha256'] = sha(HERE / 'cohort-prefix-audit.json')
    ...
    expired = manifest.get('quota_wait_until_utc')
    if expired and datetime.fromisoformat(expired) <= now():
        manifest.setdefault('expired_quota_waits', []).append(expired)
        del manifest['quota_wait_until_utc']
    ...
    for ticker in requests:
        ...
        try:
            ...
            if exc.code in (401, 403):
                manifest['provider_stop'] = 'HTTP_AUTHORIZATION_DENIED'
            if exc.code == 429:

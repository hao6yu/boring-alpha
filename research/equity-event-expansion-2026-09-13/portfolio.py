"""A cash stock account for the frozen earnings-language experiment.
...
* candidates: dictionaries with event_id, security_id, cik, accession,
  filing_date, acceptance_date_et, score, and eligibility_status. The last is
...
score threshold and uses filing date/CIK/accession. It changes no account rule.
Optional start/end narrow the evaluation window within the frozen dates, mainly
for synthetic checks; they never extend it into reserved years. Initial expense
is charged once and is included in the $5,000 initial-capital loss accounting.
"""
...
D = Decimal
ZERO, CENT = D(0), D('.01')
POLICY_SHA256 = '5c9daa1573ead42077ea323e641433c69112d6d3977f5e20c91d8bd1d04513bc'
...
def simulate(*, calendar, prices, candidates, actions=(), cost_case='base',
             rank_mode='score', start=None, end=None, initial_expense=0):
    ...
    if not ZERO <= expense < 5000:
        raise ValueError('initial expense must be in [0,5000)')
    slip = D('.001') if cost_case == 'base' else D('.005')
    cash, high = D(5000)-expense, D(5000)
    holdings, claims, decisions, fills, daily, completed = {}, [], [], [], [], []
    action_log, unresolved, candidate_gaps = [], [], []
    stopped = expense >= 1000
    ...
    for original in candidates:
        ...
        required = ('event_id', 'security_id', 'cik', 'accession', 'filing_date',
                    'acceptance_date_et', 'score', 'eligibility_status')
        ...
        available = max(iso(event['filing_date']), iso(event['acceptance_date_et']))
        index = bisect_right(calendar, available)
        ...
        if not start <= day <= end:
            ...
            continue
        if (day > policy['last_entry_date'] or index+20 >= len(calendar)
                or calendar[index+20] > end):
            decisions.append({'event_id': event['event_id'], 'date': day, 'status': 'CALENDAR_CENSORED'})
            continue
        event['exit_date'] = calendar[index+20]
    ...
    for original in actions:
        ...
        if a['type'] not in ('split', 'dividend', 'merger_cash'):
    ...
    for day in calendar[ci[start]:ci[end]+1]:
        ...
        if rank_mode == 'score':
            day_events.sort(key=lambda e: (-(e['score'] if e['score'] is not None else D('-Infinity')),
                                           e['available_date'], str(e['cik']), e['accession']))
        else:
            day_events.sort(key=lambda e: (e['available_date'], str(e['cik']), e['accession']))
        for e in day_events:
            ...
            elif len(occupied() | planned_issuers) >= 4:
                reason = 'SLOTS_FULL'
            lookback = calendar[max(0, ci[day]-60):ci[day]]
            ...
            budget = min(D(1000), cash-reserved-payable_reserve-D(1000))
        ...
        for sec, h in list(holdings.items()):
            if not (stopped or h['exit_date'] <= day or h['exit_pending']):
                continue
            h['exit_pending'] = True
            h['exit_reason'] = 'ACCOUNT_STOP' if stopped else 'SCHEDULED'
            ref = bar(day, sec)
        ...
        for e, decision, q, budget in planned:
            ...
            if spend > budget or cash-spend < 1000:
    ...
    return serial({'status': 'UNRESOLVED' if unresolved or candidate_gaps else 'COMPLETE_ACCOUNTING',
                   ...
                   'start': start, 'end': end, 'initial_capital': D(5000), 'initial_expense': expense,
                   ...
                   'action_log': action_log, 'unpriced_days': unresolved,

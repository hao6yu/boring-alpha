...
def reviewed_cover_identity(cik, page):
 security=s.common_identity(page)
 if security['status']!='UNRESOLVED_COMMON_CLASS':return security
 # These two class labels were independently resolved from original2019
 # documents before the expanded event collection, without price access.
 known={'0000055242':('KMT','0000055242-19-000016',r'^Capital Stock, par value'),
        '0001590715':('AREC','0001654954-19-009282',r'^Class A Common$')}
 if cik not in known:return security
 ticker,seed_acc,title_pattern=known[cik]
 proof=s.manual_decisions()['events'][seed_acc]
 for source in proof['sources']:
  assert s.digest(s.ROOT/source['file'])==source['sha256']
 matches=[]
 for row in page.rows:
  if (any(re.search(title_pattern,cell,re.I) for cell in row) and ticker in row
      and re.search(r'Nasdaq|New York Stock Exchange',' | '.join(row),re.I)):
   matches.append({'row_excerpt':' | '.join(row),'symbols':[ticker]})
 if len(matches)!=1:return security
 return {**security,'status':'VERIFIED_SINGLE_COMMON_CLASS','historical_symbol':ticker,
         'symbols':[ticker],'common_stock_evidence':matches,
         'class_interpretation':'Exact current cover class/ticker row corroborates the independently reviewed original2019 common-equity class; no generic capital-stock inference.',
         'original_2019_class_evidence':proof['sources']}

def build_plan():
 p=s.HERE;roster=json.loads((p/'sec-selection.json').read_text());targets={}
 # Inherited slots are immutable. Recovery only concerns the new companies.
 for issuer in roster['issuers'][100:]:
  ...
  for target in plan['targets']:
   ...
   try:
    ...
    for row in reversed(candidates[-2:]):
     ...
     b,f=client.get(base+row['primaryDocument']);security=reviewed_cover_identity(cik,s.parser.HTML(b.decode('utf8','replace')))

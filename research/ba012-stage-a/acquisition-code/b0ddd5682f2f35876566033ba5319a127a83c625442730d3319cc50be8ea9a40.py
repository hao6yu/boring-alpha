...
def run_parallel_archive(factory, quote, ceiling, output_root=OUTPUT_ROOT, workers=6):
    ...
    been durably recorded. On any failure, stop new work and reconcile all
    already submitted tasks before releasing the archive lock.
    """
    ...
    try:
        ...
        with ThreadPoolExecutor(max_workers=workers) as pool:
            while pending or next_index < len(remaining) and failure is None:
                while failure is None and len(pending) < workers and next_index < len(remaining):
                ...
                for future in ready:
                    ...
                    if phase == "metadata":
                        ...
                        if add_cost(prior, attempted, fresh, remaining_saved) > ceiling:
                            failure = "fresh_reference_estimate_budget_exceeded"
                            manifest["task_failures"].append(failure)
                            local._key = local._auth = ""
                            persist(folder, manifest)
                            continue
                        attempted = add_cost(attempted, fresh)
                        ...
                        if len(requests) == 1 and requests[0]["method"] == "timeseries.get_range" and requests[0]["query"] == query | ENCODING:
                            ...
                            error = error or "unreconciled_data_request"
                        if error:
                            manifest["task_failures"].append(error)
                            failure = failure or error
                        else:

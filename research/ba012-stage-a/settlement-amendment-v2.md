...
The JSON fixes tie handling. A single pre-price clarification supersedes the preserved initial freeze: at the final tied ordering key prefer clearing-tick precision (bit4 clear) if present, otherwise trading-tick precision; conflicts within the chosen precision remain incomplete. Chronology is never overridden by precision. Any pre-cutoff settlement DELETE conservatively invalidates the instrument/session; unsupported actions, ambiguous timestamps, conflicting ties, invalid prices and missing old/new roll endpoints remain `DATA_INCOMPLETE`. This avoids inventing undocumented deletion matching. Significant unresolved implementation complexity is a stop reason.

...
from insider_rules import truth, disclosure_day, transaction_state, linked_footnotes, disclosure_flags, classify
...
def run():
    ...
    source_audit = json.loads((HERE / 'table-projection-audit.json').read_text())
    ...
    for acc, txns in trades.items():
        ...
        for t in txns:
            ...
            if state == 'SECURITY_TITLE_REVIEW':
                ...
                hist_status = 'PS_FIELDS_VALID' if decision and decision['history']=='ALLOW' else 'NOT_COMMON' if decision and decision['history']=='EXCLUDE' else 'SECURITY_TITLE_REVIEW'
            ...
            for owner in person:
                ...
                if hist_status == 'PS_FIELDS_VALID' and public:
                    histories[cik].append({'accession':acc,'table_row':t['tableRow'],
                                           'transaction_date':t['transactionDate'],'disclosure_day':public})
                elif hist_status not in {'NOT_COMMON','EXCLUDED_NONMARKET'}:
                    uncertain[cik].append({'accession':acc,'reason':hist_status,'year':txyear,
            ...
            record = {'accession':acc,'table_row':t['tableRow'],'issuer_cik':sub['issuerCik'].zfill(10),
                      ...
                      'shares':t['transactionShares'],'reported_price':t['transactionPricePerShare'],
            ...
            elif (date.fromisoformat(public)-date.fromisoformat(t['transactionDate'])).days>90:
    ...
    for (cik,month),rows in sorted(grouped.items()):
        ...
        slots.append({'issuer_cik':cik,'disclosure_month':month,
                      'classification':next(iter(labs)) if len(labs)==1 else 'MIXED_OR_UNKNOWN',

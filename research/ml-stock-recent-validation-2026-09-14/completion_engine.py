...
def simulate(*, calendar, prices, candidates, actions=(), cost_case='base',
    ...
    for original in actions:
        ...
        if a['type'] not in ('split', 'dividend', 'merger_cash', 'stock_distribution', 'stock_merger', 'spinoff'):
    ...
    for day in calendar[ci[start]:ci[end]+1]:
        floating_missing = []
        for c in claims:
            if c['type'] == 'fractional_cash' and not c['paid']:
                if day >= c['fixed_cash_date']:
                    c['amount'] = (c['fractional_quantity'] * c['fixed_unit_cash']).quantize(CENT, rounding=ROUND_HALF_UP)
                else:
                    mark = bar(day, c['child_security_id'], allow_otc=True)
                    if mark is None:
                        floating_missing.append('fractional:' + c['child_security_id'])
                    else:
                        c['amount'] = c['fractional_quantity'] * mark
        ...
        for a in todays:
            if a['type'] in ('merger_cash', 'stock_merger', 'spinoff'):
                continue
        ...
        for a in sorted(todays, key=lambda x: x['action_id']):
            ...
            if not valid or h.get('unresolved_action'):
                ...
                continue
            if a['type'] in ('stock_merger', 'spinoff'):
                child = a['child_security_id']
                mark = bar(day, child, allow_otc=True)
                parent_mark = bar(day, sec, allow_otc=True) if a['type'] == 'spinoff' else ZERO
                if mark is None or parent_mark is None or child in holdings:
                    h['unresolved_action'] = 'unqualified_conversion:' + a['action_id']
                    continue
                total = h['quantity'] * number(a['ratio_numerator']) / number(a['ratio_denominator'])
                whole = D(int(total)); fraction = total - whole
                total_value = total * mark + h['quantity'] * parent_mark
                if total_value <= 0:
                    raise ValueError('conversion has no qualified economic value')
                basis = h['cost_basis']
                whole_basis = basis * (whole * mark) / total_value
                fraction_basis = basis * (fraction * mark) / total_value
                source_quantity = h['quantity']
                if fraction:
                    amount = fraction * mark
                    if day >= a['fixed_cash_date']:
                        amount = (fraction * number(a['fixed_unit_cash'])).quantize(CENT, rounding=ROUND_HALF_UP)
                    claims.append({'claim_id': a['action_id'] + ':fraction', 'security_id': sec,
                                   'cik': h['cik'], 'event_id': h['event_id'],
                                   'entitled_quantity': source_quantity,
                                   'originating_position_basis': fraction_basis,
                                   'amount': amount, 'type': 'fractional_cash', 'created_date': day,
                                   'cash_available_date': None, 'paid': False,
                                   'fractional_quantity': fraction, 'child_security_id': child,
                                   'fixed_cash_date': a['fixed_cash_date'],
                                   'fixed_unit_cash': number(a['fixed_unit_cash'])})
                if whole:
                    delivered = {**h, 'security_id': child, 'cik': a['child_cik'],
                                 'event_id': h['event_id'] + ':received:' + child + ':' + day,
                                 'quantity': whole, 'cost_basis': whole_basis,
                                 'raw_entry_close': mark, 'last_qualified_mark': mark,
                                 'trade_available_date': a['trade_available_date'],
                                 'origin_security_id': sec}
                    holdings[child] = delivered
                if a['type'] == 'stock_merger':
                    del holdings[sec]
                else:
                    h['cost_basis'] = basis - whole_basis - fraction_basis
                action_log.append({'date': day, 'action_id': a['action_id'], 'security_id': sec,
                                   'source_effective_date': a['effective_date'], 'type': a['type'],
                                   'source_quantity': source_quantity, 'child_security_id': child,
                                   'whole_child_quantity': whole, 'fractional_quantity': fraction})
                continue
            if a['type'] == 'split':
                h['quantity'] *= factor
        ...
        for sec, h in list(holdings.items()):
            ...
            ref = bar(day, sec)
            if ref is None or h.get('unresolved_action') or day < h.get('trade_available_date', '0001-01-01'):
                continue
            q = int(h['quantity'])
        ...
        missing, marked, holding_rows = list(floating_missing), ZERO, {}
        for sec, h in holdings.items():
        ...
        floating_claims = sum((c['amount'] for c in claims if c['type'] == 'fractional_cash' and not c['paid'] and day < c['fixed_cash_date']), ZERO)
        ...
        daily.append({'date': day, 'settled_cash': cash, 'reserved_cash_eod': ZERO,
                      'unpaid_claim_value': unpaid, 'market_sensitive_claim_value': floating_claims, 'holdings': deepcopy(holding_rows),

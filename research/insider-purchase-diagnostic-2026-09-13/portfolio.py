"""A $10,000 monthly cash account derived from the audited earnings account.
...
D = Decimal
ZERO, CENT = D(0), D('.01')
POLICY_SHA256 = '16c563cda8d5b98367f5fd9165e066227d4f58f63bc400d6cb3a7eddc4133e0b'
...
def simulate(*, calendar, prices, candidates, actions=(), cost_case='base',
    ...
    for original in candidates:
        ...
        if not start <= day <= end:
            ...
            continue
        if day != min(d for d in calendar if d[:7] == day[:7]):
            raise ValueError('Insider diagnostic entries must be first session of month')
        event['exit_date'] = max(d for d in calendar if d[:7] == day[:7] and d <= end)
    ...
    for day in calendar[ci[start]:ci[end]+1]:
        ...
        for e in day_events:
            ...
            budget = min(D(2000), D('.20')*previous_nav, cash-reserved-payable_reserve-D(2000))
